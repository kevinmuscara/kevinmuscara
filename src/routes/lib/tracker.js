    ...
    data.forEach((etrny) => {
        switch(true) {
            ...
            case coinRegex.test(entry): {
                ...
                stats.coins[game] = (typeof stats.coins[game] === 'number') ? stats.coins[game] + Number(coins) : Number(coins);
                break;
            }
        }
    });
...
function getLifetimeStats(uuid, cb) {
    getStats(uuid, (obj) => {
        ...
        return cb(`You've earned §3${xp} Exp §r, §6${coins} Coins§r and §d${karma} Karma§r using §bautotip§r`);
    });
}
